...
SELECT
	DATEADD(year, -1*(SELECT TOP 1 Offset FROM dbo._YearOffset), CAST(RegSales._Period AS date)) AS TransactionDate		-- Дата операции
	...
	,ROUND(RegSales._Fld2038/(CASE
							...
							END), 0, 1) AS QuantityInDeliveryUnits														-- Количество в единицах поступленич (кор.)
