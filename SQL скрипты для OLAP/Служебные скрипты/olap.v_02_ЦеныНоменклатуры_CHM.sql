...
IF OBJECT_ID('[dbo].[PriceListCHM]','V') IS NOT NULL
	DROP VIEW [dbo].[PriceListCHM]
GO

CREATE VIEW [dbo].[PriceListCHM]
AS
	...
	INNER JOIN dbo.Справочник_ТипыЦенНоменклатуры AS PriceType ON PriceType.Ссылка = Price.ТипЦены		-- .Справочник_ТипыЦенНоменклатуры
																AND PriceType.НаправлениеБизнеса = 0x829508606E88610311E4843B1F21A0A4
	)
	,PriceCalculation AS
	(
	SELECT
	...
	FROM Price
	LEFT JOIN olap.v_GoodsHierarchy AS Hierarchy ON Hierarchy.UID_1C = Price.ParentID AND Hierarchy.Level0 LIKE 'CHM'
	LEFT JOIN (	SELECT
					Склады.Код AS Склад
				...
				INNER JOIN dbo.Справочник_Склады AS Склады ON Склады.Ссылка = Остатки.Склад AND Склады.Код IN ('00000000425')
